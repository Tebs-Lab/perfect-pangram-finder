...
// TRUE if a word could be made using 'word' as a starting point
// FALSE otherwise
var maybe = exports.maybe = function(word, trie, cur) {
	// Lazyness
	if(find(word, trie, cur)) {
		return true;
	}
	...
	for (var node in cur) {
		// if node starts with word, or word starts with node
		// console.log(node, word);
		if (node.indexOf(word) === 0 || word.indexOf(node) === 0) {
			// val is the next node?
			var val = (typeof cur[node] === "number" && cur[node] ?
			  trie.$[cur[node]] : cur[node]);

			// how many possiblities are there?
			var possibleNext = Object.keys(val).length;
			// console.log(node, word, val);

			// If we've reached the end of word
			// return true if it's an exact match or we can continue
			if (node.length >= word.length){
				return val === 0 || val.$ === 0 || possibleNext > 0;	
			}

			// if we cant continue, quit!
			else if(possibleNext === 0) {
				return false;
			}
 
			// There are more nodes, continue
			else {
				return maybe(word.slice(node.length), trie, val);
			}
		}
	}
