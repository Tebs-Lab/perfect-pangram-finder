...
function search(allLetters, _CONFIG) {
	...
	while(!frontier.isEmpty()) {
		...
		// Prune nodes from the frontier which match currentNode's letters
		...
		// Clearly we can't have cycles, but we want to capture the unique paths
		// to current node instead of simply discarding them.
		while(!currentNode || explored.has(currentNode.letters)) {
 ...
 */
const EXPERIMENTAL_MAX_FRONTIER_SIZE = Infinity;
