...
const PangramHeuristic = require('./pangram-heuristic');
...
class PangramHeuristicGraphSearch {
  constructor(alphabet, dictionary, explorationRate = 0) {
    ...
    this._heuristicGenerator = new PangramHeuristic(this._alphabet, this._dictionary.keys());
  ...
  */
  _expandFrontier(expansionNode) {
    ...
    for(let word of validWords) {
      ...
      let utility = this._heuristicGenerator.computeWeightedHeuristic(newLetters);
  ...
  _handleSolutionCandidate(node, solutionWords) {
    ...
    if(node === this._rootNode) {
      ...
      if(!this._knownPerfectPangrams.has(cannonizedPangram)) {
        ...
        if(this._knownPerfectPangrams.size % 100 === 0) console.log(this._knownPerfectPangrams.size);
      }
    }
